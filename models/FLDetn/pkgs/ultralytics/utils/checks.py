...
from models.FLDetn.pkgs.ultralytics.utils import (ASSETS, AUTOINSTALL, LINUX, LOGGER, ONLINE, ROOT, USER_CONFIG_DIR, SimpleNamespace,
...
def parse_version(v='0.0.0') -> tuple:
    ...
    """
    # find first non-digit/dot, slice there
    correct = [True if x == '.' else x.isdigit() for x in v]
    if False in correct:
        v = v[:correct.index(False)]
    # strip any trailing dot left by slicing (e.g. "2.8.0.")
    v = v.rstrip('.')
    return tuple(map(int, v.split('.')))  # e.g. "2.8.0.devXYZ" -> "2.8.0" -> (2, 8, 0)
...
def collect_system_info():
    ...
    from models.FLDetn.pkgs.ultralytics.utils import ENVIRONMENT, is_git_dir
    from models.FLDetn.pkgs.ultralytics.utils.torch_utils import get_cpu_info
...
def check_amp(model):
    ...
    Example:
        ...
        from models.FLDetn.pkgs.ultralytics.utils.checks import check_amp
